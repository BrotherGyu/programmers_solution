...
## 2018 KAKAO BLIND RECRUITMENT [re, deque]
"""
문제 설명
뉴스 클러스터링
...
def solution(str1,str2):
    ...
    for index, value in enumerate(str1[:-1]):
        if value!=' ' and str1[index+1]!=' ':
            str1_li.append(value+str1[index+1])
    for index, value in enumerate(str2[:-1]):
        if value!=' ' and str2[index+1]!=' ':
            str2_li.append(value+str2[index+1])
    str1_que, str2_que = deque(sorted(str1_li)), deque(sorted(str2_li))
    while True:
        if len(str1_que)!=0 and len(str2_que)!=0:
            if str1_que[0]==str2_que[0]:
                intersection.append(str1_que.popleft())
                union.append(str2_que.popleft())
            elif str1_que[0]>str2_que[0]:
                union.append(str2_que.popleft())
            else:
                union.append(str1_que.popleft())
        else:
            if len(str1_que)!=0:
                while len(str1_que)!=0:
                    union.append(str1_que.popleft())
                break
            else:
                while len(str2_que)!=0:
                    union.append(str2_que.popleft())
                break
    if len(union)==0:
        return 65536
