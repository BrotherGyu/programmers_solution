...
def solution(n, words):
    ...
    for i in range(len(words)-1):
        words_i, words_next=words[i],words[i+1]
        counter+=Counter([words_next])
        if words_i[-1]!=words_next[0] or counter[words_next]>1:
...
테스트 11 〉통과 (0.22ms, 9.97MB)
테스트 12 〉통과 (0.09ms, 9.98MB)
