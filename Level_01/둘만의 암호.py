...
def solution(s, skip, index):
    ...
    for pop_value in skip:
        alpha[ord(pop_value)%97] = None
    for input_value in s:
        alpha_que = deque(alpha)
        alph_index = ord(input_value)%97
        cnt = 0
        while index != cnt:
            alpha_que.rotate(-1)
            while alpha_que[alph_index] == None:
                alpha_que.rotate(-1)
            cnt += 1
        answer += alpha_que[alph_index]
    return answer